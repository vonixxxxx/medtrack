import { useState, useEffect } from "react";
import { motion, useReducedMotion } from "framer-motion";
import { Pill, Plus, Calendar, Clock } from "lucide-react";
import DashboardCard from "../DashboardCard";
import { Button } from "../ui/button";
import { Badge } from "../ui/badge";
import { LoadingSkeleton } from "../dashboard/LoadingSkeleton";
import { EmptyState } from "../dashboard/EmptyState";
import { getPrescriptions } from "../../api";

export const PrescriptionList = ({ patientId, onAddPrescription, refreshTrigger, readOnly = false }) => {
  const prefersReducedMotion = useReducedMotion();
  const [prescriptions, setPrescriptions] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    fetchPrescriptions();
  }, [patientId, refreshTrigger]);

  const fetchPrescriptions = async () => {
    try {
      setIsLoading(true);
      const params = patientId ? { patientId } : {};
      const data = await getPrescriptions(params);
      // Ensure data is always an array
      const prescriptionsArray = Array.isArray(data) ? data : [];
      setPrescriptions(prescriptionsArray);
      setError(null);
    } catch (err) {
      console.error('Error fetching prescriptions:', err);
      setError('Failed to load prescriptions');
      setPrescriptions([]);
    } finally {
      setIsLoading(false);
    }
  };

  const formatDate = (dateString) => {
    if (!dateString) return null;
    const date = new Date(dateString);
    return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  };

  const getStatusColor = (status) => {
    const colors = {
      active: 'bg-medical-50 text-medical-700',
      filled: 'bg-primary-50 text-primary-700',
      cancelled: 'bg-error-50 text-error-700',
      expired: 'bg-neutral-100 text-neutral-600'
    };
    return colors[status] || 'bg-neutral-50 text-neutral-700';
  };

  return (
    <DashboardCard
      title="Prescriptions"
      icon={<Pill size={20} />}
      variant={readOnly ? "patient" : "clinician"}
      action={
        !readOnly && onAddPrescription && (
          <Button onClick={onAddPrescription} variant="primary" size="sm">
            <Plus size={16} className="mr-1.5" />
            New Prescription
          </Button>
        )
      }
    >
      {isLoading ? (
        <LoadingSkeleton variant="list" count={3} />
      ) : error ? (
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="text-center py-12"
        >
          <div className="mb-4 p-3 bg-error-50 rounded-2xl inline-block">
            <p className="text-error-600 font-medium text-sm">{error}</p>
          </div>
          <Button onClick={fetchPrescriptions} variant="secondary" size="sm">
            Try again
          </Button>
        </motion.div>
      ) : (Array.isArray(prescriptions) ? prescriptions : []).length === 0 ? (
        <EmptyState
          icon={Pill}
          title="No prescriptions"
          description={readOnly ? "Your prescription history is empty" : "Add prescriptions to manage patient medications"}
          action={!readOnly && onAddPrescription ? {
            label: "New Prescription",
            onClick: onAddPrescription
          } : null}
        />
      ) : (
        <div className="space-y-3">
          {(Array.isArray(prescriptions) ? prescriptions : []).map((prescription, index) => (
            <motion.div
              key={prescription.id || index}
              initial={{ opacity: 0, y: prefersReducedMotion ? 0 : 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{
                duration: 0.2,
                delay: index * 0.03,
                ease: [0.16, 1, 0.3, 1]
              }}
              whileHover={prefersReducedMotion ? {} : { y: -2 }}
              className="group p-4 bg-white rounded-2xl border border-neutral-200 hover:border-primary-300 hover:shadow-medium transition-all duration-200"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <h4 className="font-semibold text-neutral-900 text-base">
                      {prescription.medicationName}
                    </h4>
                    <Badge className={`text-xs ${getStatusColor(prescription.status)} border-0 font-medium`}>
                      {prescription.status}
                    </Badge>
                  </div>
                  
                  <div className="space-y-1 text-sm text-neutral-600">
                    {prescription.dosage && (
                      <p><span className="font-medium">Dosage:</span> {prescription.dosage} {prescription.unit || ''}</p>
                    )}
                    {prescription.frequency && (
                      <p><span className="font-medium">Frequency:</span> {prescription.frequency}</p>
                    )}
                    {prescription.route && (
                      <p><span className="font-medium">Route:</span> {prescription.route}</p>
                    )}
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-neutral-400" />
                      <span>Prescribed: {formatDate(prescription.datePrescribed)}</span>
                    </div>
                    {prescription.refills > 0 && (
                      <p><span className="font-medium">Refills:</span> {prescription.refills}</p>
                    )}
                    {prescription.instructions && (
                      <p className="text-neutral-500 mt-2"><span className="font-medium">Instructions:</span> {prescription.instructions}</p>
                    )}
                  </div>
                </div>
              </div>
            </motion.div>
          ))}
        </div>
      )}
    </DashboardCard>
  );
};

